...
from .utility.misc import entropy, get_rsfx, saveOracle
...
class FO(FactorOracle):
    ...
    def latent(self):
        ...
        for s in self.trn[0]:
            indices = {s}
            indices = get_rsfx(self, indices, s)
...
class MO(FactorOracle):
    ...
    def add_state(self, new_data, method='inc'):
        ...
        while k is not None:
            ...
            else:       # ici, c'est le calcul de la distance entre new_data qui est le vecteur des features du nouvel
                # état, et f_array[trn(k)] le tableau des features des liens en avant de k. On les récupères bien tous
                # car on récupère uniquement la première ligne, mais il n'y en avait qu'une (l'opération tab[0] permet
                # d'enlever des crochets superflus
                dvec = dist.cdist([new_data],
                                  self.f_array[self.trn[k]],
                                  metric=self.params['dfunc'])[0]
            print("dvec :", dvec)
            print("threshold :", self.params['threshold'])
            I = np.where(dvec < self.params['threshold'])[0]  # retourne les indices du tableau
            print("I :", I)
            # dvec pour lesquels la condition dvec < threshold est vraie
            if len(I) == 0:  # if no similar transition from suffix
                self.trn[k].append(i)  # Add new forward link to unvisited state
                print("k, forward link k: ", k, self.trn[k])
                pi_1 = k
                if method != 'complete':
                    k = self.sfx[k]
            else:  # si il y a au moins une transition qui est considérée comme similaire à partir du suffixe
                if method == 'inc':
                    if I.shape[0] == 1:  # s'il n'y en a qu'une seule, alors le suffixe est celle-ci
                        suffix_candidate = self.trn[k][I[0]]
                    else:  # Sinon on prend la meilleure des transition (la distance la moins élevée)
                        suffix_candidate = self.trn[k][I[np.argmin(dvec[I])]]
                    break  # pour la méthode 'inc', on s'arrête au premier suffixe dont un lien a été trouvé
                elif method == 'complete':  # prend le lien en question et la valeur de sa distance avec l'état actuel
                    suffix_candidate.append((self.trn[k][I[np.argmin(dvec[I])]],
                                             np.min(dvec)))
                    print("suffix candidates :", suffix_candidate)
        ...
        if method == 'complete':  # Si on a un nouveau matériau
            if not suffix_candidate:
                ...
                sorted_suffix_candidates = sorted(suffix_candidate,
                                                  key=lambda suffix: suffix[1])  # trie avec le 2e élément de chaque
                print("sorted suffix cadidates :", sorted_suffix_candidates)
                # couple par ordre croissant
