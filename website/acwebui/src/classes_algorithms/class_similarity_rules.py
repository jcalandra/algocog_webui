...
def similarity(ms_oracle, level, params):
    if level > 0:
        matrix = ms_oracle.levels[level - 1].materials.sim_matrix
    else:
        matrix = ms_oracle.matrix
...
def char_next_level_similarity(ms_oracle, level, params):
    ...
    new_rep, sim_tab, sim_digit = similarity(ms_oracle, level, params)
