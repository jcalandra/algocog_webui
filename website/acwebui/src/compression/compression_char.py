...
def convert(fd_str, lvl, mso, t_start, t_end):
    ...
    for i in range(t_start, t_end):
        fd_str = fd_str + chr(oracle.data[i] + 96)
    return fd_str
...
def already_seen(lvl, m_object):
    ...
    global nb_materials
    if ord(m_object) > nb_materials[lvl]:
        nb_materials[lvl] = ord(m_object)
        return 0
    return 1
...
def add_dict(level, char):
    ...
    level. """
    global dictionary
    if char == '(' or (level == 0 and len(dictionary[0]) < ord(char) - 96):
        char = chr(len(dictionary[level]) + 97)
        dictionary[level].append([char, ''])
    if level < len(dictionary) - 1:
        dictionary[level + 1][len(dictionary[level + 1]) - 1][1] += char
    return char
...
def add_recursive_df(level, char):
    ...
    add_df(level, char)
    if level > 0:
        for i in range(len(dictionary[level][ord(char) - 97][1])):
            add_recursive_df(level - 1, dictionary[level][ord(char) - 97][1][i])
        level -= 1
    return 0
