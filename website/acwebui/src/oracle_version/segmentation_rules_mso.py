...
from .similarity_computation import compute_alignment
from .formal_diagram_mso import *
from acwebui.src.parameters import ALIGNEMENT_rule3, ALIGNEMENT_rule4, LETTER_DIFF
from .algo_segmentation_mso import structure
...
def rule_3_existing_object(history_next, concat_obj, actual_char, matrix, params):
    ...
    if ALIGNEMENT_rule3:
        for i in range(len(history_next)):
            if compute_alignment(history_next[i][1], concat_obj, matrix, params)[0] and  \
compute_alignment(history_next[i][1], concat_obj + chr(actual_char + letter_diff), matrix, params)[0] == 0:
                return 1
    else:
        for i in range(len(history_next)):
...
def rule_4_recomputed_object(oracles, matrix, level, actual_char_ind, str_obj, k, level_max, end_mk, params):
    """ This function compare the actual concatenated object concat_obj of unstructured characters of the actual level
    ...
    if ALIGNEMENT_rule4:
        for j in range(nb_elements):
            ...
            # if there is a difference between concat_obj and the longest similar suffix of the first char of concat_obj
        if compute_alignment(sub_suffix, concat_obj, matrix, params)[0] == 0:
            return 0, str_obj
    ...
    if ALIGNEMENT_rule4:
        if compute_alignment(sub_suffix, concat_obj + chr(f_oracle.data[actual_char_ind] + letter_diff),
                                                    matrix, params)[0] == 1:
            return 0, str_obj
    else:
    ...
    if len(oracles[1]) > level + 1:
        ...
        if ALIGNEMENT_rule4:
            if compute_alignment(
                    history_next[real_value - 1][1], concat_obj, matrix, params)[0] == 1 or \
                    compute_alignment(
                    history_next[real_value - 1][1], sub_suffix, matrix, params)[0] == 1:
                return 0, str_obj
        else:
    ...
    if to_struct:
        for j in range(ind_to_struct):
            ...
            formal_diagram_update(oracles[1][level][4], data_length, new_state, char_ind, oracles, level, params)
            print_formal_diagram_update(oracles[1][level][5], level, oracles[1][level][4], data_length)
        structure(to_struct_obj, oracles, level, oracles[1][level][1], data_length, level_max, end_mk, params)
        oracles[1][level][3] = ""
