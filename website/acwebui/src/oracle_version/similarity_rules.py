from .similarity_computation import compute_alignment, quotient, compute_window_audio, compute_descriptor, \
    compute_signal_similarity
from acwebui.src.parameters import LETTER_DIFF, ALIGNMENT, STRICT_EQUALITY
...
def similarity_alignment(oracles, level, params):
    history_next = oracles[1][level][2]
    concat_obj = oracles[1][level][3]
    if level > 0:
    ...
    sim_tab = []
    for i in range(len(history_next)):
        sim_digit, sim_value = compute_alignment(history_next[i][1], concat_obj, matrix, params)
        sim_tab.append(sim_value / quotient)
        if sim_digit:
            new_char = history_next[i][0]
            return new_char, None, sim_tab, 1
    return None, None, sim_tab, 0
...
def similarity_signal(oracles, level, params):
    sim_tab = []
    s_tab = []
    history_next = oracles[1][level][2]
    concat_obj = oracles[1][level][3]
    window = compute_window_audio(oracles, level, concat_obj)
    actual_object_descriptor = compute_descriptor(window)
    ...
    for i in range(len(history_next)):
        ...
        sim_digit, sim_value = compute_signal_similarity(s_tab, i, params)
...
def char_next_level_similarity(oracles, level, params):
    ...
    if STRICT_EQUALITY:
        ...
        new_char, new_descriptor, sim_tab, digit = similarity_alignment(oracles, level, params)
    else:
        new_char, new_descriptor, sim_tab, digit = similarity_alignment(oracles, level, params)
    if digit:
        return new_char
