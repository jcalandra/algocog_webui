...
import numpy as np
from acwebui.lib.vmo_master.vmo.VMO.utility.misc import entropy, get_rsfx, saveOracle
from .similarity_functions import frequency_static_similarity
from acwebui.src.parameters import PARCOURS, INCERTITUDE
...
class MO(FactorOracle):
    ...
    def add_state(self, new_data, s_tab, v_tab, params, method='inc'):
        ...
        while k is not None:
            ...
            for j in range(len(self.trn[k])):
                fss = frequency_static_similarity(s_tab, self.trn[k][j] - 1, i - 1)
                dvec.append(fss)
        ...
        if n > 0:
            ...
            J = []
            for j in range(n):
                fss = frequency_static_similarity(compare_tab_rep, j, i - 1 + n)
                comp_rep.append(fss)
                    ...
                    if (method == 'complete' and len(sorted_suffix_candidates) > 0 and
                       ...
                       (not suffix_candidate):
                        for j in range(len(self.latent[mat_rep])):
                            ...
                            fss = frequency_static_similarity(s_tab, actual_compared - 1, i - 1)
...
def _build_oracle(flag, oracle, input_data, volume_data, params, suffix_method='inc'):
    ...
    if flag == 'a':
        [oracle.add_state(obs, input_data, volume_data, params, suffix_method) for obs in input_data]
...
def build_oracle(input_data, params, volume_data, flag='a',
    ...
    if flag == 'a':
        oracle = _create_oracle(flag, threshold=threshold, dfunc=dfunc,
                                dfunc_handle=dfunc_handle, dim=dim)
        oracle = _build_oracle(flag, oracle, input_data, volume_data, params, suffix_method)
    elif flag == 'f' or flag == 'v':
        oracle = _create_oracle(flag, threshold=threshold, dfunc=dfunc,
                                dfunc_handle=dfunc_handle, dim=dim)
        oracle = _build_oracle(flag, oracle, input_data, volume_data, params)
    else:
